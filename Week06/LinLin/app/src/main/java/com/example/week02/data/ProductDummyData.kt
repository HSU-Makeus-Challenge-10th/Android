...
object ProductDummyData {
    ...
    /**
     * 로컬 JSON에 저장된 [ProductItem.imageResId]는 빌드마다 달라질 수 있어,
     * UI에서는 항상 여기서 id 기준으로 이미지 리소스를 맞춘다.
     */
    fun catalogItemById(id: Int): ProductItem? =
        homeNewProducts().firstOrNull { it.id == id }
            ?: shopProducts().firstOrNull { it.id == id }
            ?: wishlistProducts().firstOrNull { it.id == id }
