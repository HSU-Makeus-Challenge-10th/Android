// 홈 화면 UI로, 브랜드 영역과 최신 상품 섹션 및 상단 이동 기능을 구성합니다.
...
fun HomeScreen(
    ...
    // 날짜는 화면 진입 시점 기준으로 보여주면 충분하므로 재구성마다 다시 포맷하지 않습니다.
    ...
    Box(
        ...
        // 홈 콘텐츠를 하나의 LazyColumn에 모아 스크롤 상태와 상단 이동 동작을 한 곳에서 관리합니다.
        LazyColumn(
            state = listState,
            ...
            // contentType을 섹션별로 지정해 Lazy 레이아웃이 서로 다른 UI를 잘못 재사용하지 않게 합니다.
            ...
            item(contentType = "latestProducts") {
                LazyRow(
                    ...
                    items(
                        // 홈에서는 최신 상품만 보여주고, 전체 상품 탐색은 Shop 화면이 담당합니다.
        ...
        if (showScrollToTop) {
            Button(
                onClick = {
                    // animateScrollToItem은 suspend 함수라 Composable 생명주기에 묶인 scope에서 실행합니다.
